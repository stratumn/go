...
func TestMatchScheme(t *testing.T) {
	...
	t.Run("Unknown scheme", func(t *testing.T) {
		...
		assert.EqualError(t, err, "Unhandled signature schema [test]: signature type must be one of [ED25519 ECDSA256 RSA]")
		assert.Empty(t, got)
	})
}
func TestVerify(t *testing.T) {
	...
	t.Run("RSA", func(t *testing.T) {
		...
		t.Run("Bad public key format", func(t *testing.T) {
			...
			assert.EqualError(t, err, "Could not parse RSA public key, wrong (N, E) parameters")
		})
	})
}
