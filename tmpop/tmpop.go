...
func (t *TMPop) Commit() abci.ResponseCommit {
	...
	if err != nil {
		log.Errorf("Error while committing: %s", err.Error())
		return abci.ResponseCommit{}
	}

	if err := t.saveValidatorHash(); err != nil {
		log.Errorf("Error while saving validator hash: %s", err.Error())
		return abci.ResponseCommit{}
	}

	if err := t.saveCommitLinkHashes(links); err != nil {
		log.Errorf("Error while saving committed link hashes: %s", err.Error())
		return abci.ResponseCommit{}
	}
