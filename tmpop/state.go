...
func NewState(a store.Adapter, config *Config) (*State, error) {
	...
	if config.ValidatorFilename != "" {
		...
		if watcher, err = fsnotify.NewWatcher(); err != nil {
			log.Error(errors.Wrap(err, "cannot create a new filesystem watcher for validators"))
		}
		if err = watcher.Add(config.ValidatorFilename); err != nil {
			log.Error(errors.Wrapf(err, "cannot watch validator configuration file %s", config.ValidatorFilename))
		}
	}
...
func (s *State) UpdateValidators() {
	if s.validatorWatcher != nil {
		...
		if validatorFile != "" {
			loadValidatorFile(validatorFile, s.validatorChan)
		}
	}
	select {
	case validator := <-s.validatorChan:
		log.Info("validator received a new validator")
