...
func (s *Store) UpdateValidator(ctx context.Context, link *cs.Link) error {
	...
	lastRulesLink := segments[0].Link
	if canonicalCompare(link.State, lastRulesLink.State) != nil {
		log.Infof("Validator of process %s has to be updated in store", process)
		if link.Meta.Priority <= lastRulesLink.Meta.Priority {
			return ErrBadPriority
		}
		lastRulesLinkHash, _ := lastRulesLink.HashString()
		if link.Meta.PrevLinkHash != lastRulesLinkHash {
			return ErrBadPrevLinkHash
		}
		if link.Meta.MapID != lastRulesLink.Meta.MapID {
			return ErrBadMapID
		}
		if process != lastRulesLink.Meta.Data[ProcessMetaKey] {
...
func (s *Store) LinkFromSchema(ctx context.Context, process string, schema *RulesSchema) (*cs.Link, error) {
	...
	if len(segments) > 0 {
		lastRulesLink = segments[0].Link
		priority = lastRulesLink.Meta.Priority + 1.
		mapID = lastRulesLink.Meta.MapID
		var err error
		if prevLinkHash, err = lastRulesLink.HashString(); err != nil {
